...
scan_dir = 'Fig_2i_23.1mrad_Talos'; % change this for different scans
...
df = -50*10*-1*0.798; %defocus in angstrom, the sign is oppisite to UI defocus, and there's a 1.43 factor for actual defocus at 80kV for Themis
ADU = 393; % Depends on kV, calibrated from the orignal EMPAD paper: doi:10.1017/S1431927615015664, 80kV: 151, 200kV: 393
voltage = 200;
alpha0 = 23.07; %convergence semi angle # 18->18.44, 25.2->24.92, 36->36.4, 54->55.45
rbf = 24.43; %radius of central disk in px at given camera length. 16 - cl 230 mm; 
rot_ang = 0; %angle between cbed and scan coord.
scan_step_size = 0.3920; % For 128x128 scan positions, 1.182 - 5.1 Mx; 0.839 - 7.2 Mx; 0.605 - 10 Mx; 0.429 - 14.5Mx; 0.297 - 20.5 Mx; 0.208 - 29 Mx; 0.147 - 41 Mx %angstrom
