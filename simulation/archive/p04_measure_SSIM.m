...
save_dir = 'p02_pipeline/20231221_matlab_ssim_lines';
mkdir(save_dir);

base_dir = strcat('H:/20231219_ptycho_recon_kmax2/'); % Where you store the reconstruction data
base_dir2 = 'C:/Users/chiahao3/Box/01_Project_Analysis/2022_Q4_Ptychography/20231121_ptycho_paper_better_sim/'; % Where you store the ground truth file
ground_truth_path = 'transmission_static_dx0.08Ang.mat';
ground_truth_scaling = 3; % The loaded ground truth would be rescale based on this factor. If it's 3, it means the ground truth would be shrunk to 1/3 of the original pixel number (3x larger physical size of the pixel)

mode_dirs = {'static_coherent', 'static_temporal_spatial'}; %static_temporal_spatial
alpha_max_s = {'1.0','3.0','5.0','7.5','9.5','10.5','12.0','15.0','18.0', ...
    '20.0', '22.0', '23.1', '24.0', '26.0', '28.0', '30.0', '32.0', '34.0', '36.0', '38.0', '40.0'}; %convergence angle in mrad
Cs_s = {'Cs_1.0um_', 'Cs_1.1mm_'};
N_s = [128, 256, 512]; % size of diffraction pattern in pixel. only square dp allowed
postfixs = {'_blur_0px_dose_1.0e+06ePerAng2', '_blur_0px_dose_1.0e+07ePerAng2', '_blur_0px_dose_1.0e+08ePerAng2'};
dose_s = [1e6, 1e7, 1e8];
        ...
        for jj = 1:length(Cs_s)
            ...
            for kk = 1:length(N_s)
                ...
                for ll = 1:length(alpha_max_s)
                    ...
                    disp(data_dir)
                    recon_dir = strcat('1\roi_1_Ndp_', num2str(N), '/MLs_L1_p16_g128_dpFlip_T/');
                    %%
                    Niter = 500;
